 ...
 * de-csrf
 *
 * Clone of connect-csrf adjusted to generate tokens once per session instead
 * of once per request.
 *
 * MIT Licensed
 */
  ...
  return function(req, res, next){
    ...
    function createToken(secret) {
      ...
      req.csrfToken = function csrfToken() {
        return token || (token = req.session.csrfToken) || (token = req.session.csrfToken = saltedToken(secret));
      };
